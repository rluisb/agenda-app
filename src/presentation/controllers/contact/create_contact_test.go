...
func TestCreateContactWithSuccess(t *testing.T) {
	...
	if w.Code == http.StatusOK {
		expectedResponse := *models.NewContactModel("1", contact.Name, contact.Email, contact.Phone, contact.Address)
		var responseBody models.ContactModel
		json.Unmarshal(w.Body.Bytes(), &responseBody)
		if expectedResponse != responseBody{
			t.Errorf("Expected response body %v, got %v", expectedResponse, expectedResponse)
		}
		if w.Header()["Content-Type"][0] != "application/json" {
			t.Errorf("Expected response header Content-Type to be application/json, got %v", w.Header()["Content-Type"][0])
		}
		if emailValidatorSpy.CallCount != 1 {
